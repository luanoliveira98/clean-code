...
function checkIfStudentPassedTheSchoolYear() {
  const isStudentNotApprovedByGrade = studentGrade < necessaryGradeToBeApproved;
  const isStudentNotApprovedByAbsenses = studentNumberOfAbsenses > numberOfAbsensesToFailSchool;

  if (!isStudentNotApprovedByGrade) {
    return { error: true, message: 'Student was not approved because his grade was below the necessary.'}
  } 

  if(isStudentNotApprovedByAbsenses) {
